...
async def exchange_token(request: TokenExchangeRequest):
    ...
    """
    token_info = await meta_oauth_service.exchange_code_for_token(request.code)
    if not token_info:
        raise HTTPException(status_code=400, detail="Failed to exchange code for token")
...
async def extend_token(request: TokenRefreshRequest):
    """
    Exchange a short-lived token for a long-lived token (60 days).

    Call this immediately after getting a short-lived token.
    """
    long_lived = await meta_oauth_service.exchange_for_long_lived_token(request.access_token)
    if not long_lived:
...
async def debug_token(request: TokenDebugRequest):
    ...
    """
    info = await meta_oauth_service.debug_token(request.access_token)
...
async def get_ad_accounts(request: AdAccountRequest):
    """
...
async def get_campaigns(request: AdAccountRequest):
    """
...
async def get_campaign_insights(request: CampaignInsightsRequest):
    """
...
@router.get("/status")
async def get_meta_integration_status():
    """
